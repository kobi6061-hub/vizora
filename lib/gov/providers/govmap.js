...
class GovMapProvider extends GovernmentRealEstateProvider {
  ...
  normalizeDeal(raw, ctx = {}) {
    ...
    const tx = makeTransaction({
      ...
      city: norm(raw.settlementNameHeb) || norm(poly.settlementNameHeb) || null,
      /* the deal row often omits the address; the polygon it came from IS
         the building, so its registry address is carried over */
      street: norm(raw.streetName) || norm(poly.streetNameHeb) || null,
      houseNumber: raw.houseNumber != null ? norm(raw.houseNumber)
        : (poly.houseNum != null ? norm(poly.houseNum) : null),
  ...
  async dealsAroundPoint(x, y, radiusM, target, filters) {
    ...
    for (const p of polygons) {
      ...
      if (polys.length >= (filters.maxPolygons || 8)) break; // bounded fan-out
    }
    ...
    for (const poly of polys) {
      for (const dealType of [1, 2]) { // government classes: first hand + second hand
        try {
          ...
          polyErrors++; // a polygon/class miss must not sink the whole query
        }
      }
    }
